...
Docs / 文档: [README](https://github.com/dageno-agents/seo-geo-audit) · [简体中文](https://github.com/dageno-agents/seo-geo-audit/blob/main/README.zh-CN.md)
...
> 一套面向品牌、SaaS、服务商和增长团队的 SEO + GEO 诊断框架。

## 它解决什么问题

- 传统 SEO audit 往往只看技术 SEO 或页面问题，很难回答“这个网站能不能被 AI 搜索理解、信任和引用”。
- 这个项目把技术健康、内容质量、信任信号、站外实体、AI 可见性放进同一份诊断里。

## 什么时候用它

- 售前阶段需要快速判断客户网站问题。
- 要给老板、运营、专家分别输出不同深度的报告。
- 要把 SEO 问题和 GEO/AI visibility 问题放在同一套语言里解释。

## 和相邻项目有什么区别

- 它是“诊断体检”，先告诉你哪里有问题。
- `geo-site-architecture-audit` 更偏站点结构和页面体系怎么补。
- `seo-geo-content-engine` 更偏诊断后怎么批量生产内容。

## 主要输出

- Boss mode executive report
...
1. 从 `SKILL.md` 开始了解使用入口。
2. 按客户类型选择 crawl、report depth 和 output mode。
3. 优先用 Boss mode 做销售沟通，再用 Operator/Specialist mode 做执行。
